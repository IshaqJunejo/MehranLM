...
def train_bpe(corpus: str, num_merges: int):
    # Adding the speical token for NEW LINE
    corpus = corpus.replace("\n", "<NL>")
    preprocessed_corpus = " ".join([word + "<W>" for word in corpus.split()])
...
def encode(text: str, tokenizer) -> List[int]:
    text = text.replace("\n", "<NL>")
    preprocessed_text = " ".join([word + "<W>" for word in text.split()])
    return tokenizer.encode(preprocessed_text).ids

def decode(ids: List[int], tokenizer) -> str:
    decoded_text = tokenizer.decode(ids)
    return decoded_text.replace("<PAD>", "").replace(" ", "").replace("<W>", " ").replace("<NL>", "\n")
